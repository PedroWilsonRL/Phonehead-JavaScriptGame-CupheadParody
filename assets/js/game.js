...
function animate(timeStamp) {
    ...
    if (startSequenceTimer < 180) {
    const centerX = canvas.width / 2;
    const centerY = canvas.height / 2;

    if (startSequenceTimer < 120) {
        const maxWidth = 800;
        const maxHeight = 600;
        const progress = Math.min(startSequenceTimer / 120, 1);
        ...
        const imgWidth = 900;
        const imgHeight = 700;
        ctx.drawImage(goImage, centerX - imgWidth / 2, centerY - imgHeight / 2, imgWidth, imgHeight);
    }
    ...
    if (player.facingLeft) {
        ctx.save();
        ctx.scale(-1, 1);
        ctx.drawImage(player.image, player.frameX * frameWidth, player.frameY * frameHeight, frameWidth, frameHeight, -player.x - player.width, player.y, player.width, player.height);
        ctx.restore();
    } else {
        ctx.drawImage(player.image, player.frameX * frameWidth, player.frameY * frameHeight, frameWidth, frameHeight, player.x, player.y, player.width, player.height);
    }
    ...
    for (let i = fireballs.length - 1; i >= 0; i--) {
        ...
        ctx.drawImage(fireballImage, f.x, f.y, f.width, f.height);
    }

    updateEnemy();
    handleEnemyAnimation(deltaTime);
    ctx.drawImage(enemy.image, enemy.frameX * enemyFrameWidth, enemy.frameY * enemyFrameHeight, enemyFrameWidth, enemyFrameHeight, enemy.x, enemy.y, enemy.width, enemy.height);
    ...
    for (let i = enemyFireballs.length - 1; i >= 0; i--) {
        ...
        ctx.drawImage(enemyFireballImage, f.x, f.y, f.width, f.height);
    }
